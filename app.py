Corrections : --ignore-certificate-errors, ignore_https_errors=True, http:// au lieu de https://
...
def generate_barcode():
 ...
 # Utiliser HTTP au lieu de HTTPS pour eviter les erreurs SSL
 ...
 'input[type="search"], input[placeholder*="cherch"]'
 )
 if search_input:
 search_input.fill(match_name.split(' vs ')[0])
 time.sleep(2)
 prediction_btn = page.query_selector(
 f'[data-outcome="{prono}"], button:has-text("{prono}")'
 )
 if prediction_btn:
 prediction_btn.click()
 time.sleep(1)
 mise_input = page.query_selector(
 'input[name="mise"], input[placeholder*="mise"]'
 )
 if mise_input:
 mise_input.fill(str(mise))
 generate_btn = page.query_selector(
 'button:has-text("Generer"), button:has-text("Reserver")'
 )
 if generate_btn:
 generate_btn.click()
 time.sleep(5)
